...
export function FactoModal({ isOpen, onClose }: FactoModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('http://localhost:4000/api/facto/analyze', {
        method: 'POST',
      ...
      if (data.ok) {
        ...
        setResult(`<p style="color:red;">분석 실패: ${data.error}</p>`);
      }
    } catch (err) {
      setIsProcessing(false);
      setResult(`<p style="color:red;">Facto 오류 발생</p>`);
    }
  };
      ...
      >
        <div onClick={(e) => e.stopPropagation()}>
          ...
          >
            ...
            {/* 결과 표시 영역 */}
            {result && (
              <div className="mt-12 p-6 border border-gray-200 dark:border-gray-800 rounded bg-gray-50 dark:bg-gray-900 overflow-x-auto break-words">
                <article
