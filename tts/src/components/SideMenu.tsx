import { Home, TrendingUp, Bookmark, Settings, X, Search, History } from 'lucide-react';
...
export function SideMenu({ isOpen, onClose, onFactoOpen, onSettingsOpen, onHistoryOpen, isLoggedIn }: SideMenuProps) {
  const { t } = useLanguage();
  ...
  const menuItems = [
    ...
    { icon: History, label: t('menu.history') || 'Facto 기록', href: '#', onClick: () => { 
      if (isLoggedIn) {
        onHistoryOpen(); 
        onClose();
      }
    } },
    { icon: Settings, label: t('menu.settings'), href: '#', onClick: () => { 
      if (isLoggedIn) {
        onSettingsOpen(); 
        onClose();
      }
